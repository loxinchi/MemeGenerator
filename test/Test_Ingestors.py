...
from QuoteEngine.exceptions import WrongIngestorError, UnsupportedFileTypeError
...
class Test_Ingestors():
    ...
    def test_ingestors_by_type(self, ingestor_type: str, file_name: str, quotes_root) -> None:
    ...
    @pytest.mark.parametrize("ingestor_type, file_name", [(CSVIngestor, "DogQuotesDOCX.docx"),
                                                          (DOCXIngestor, "DogQuotesPDF.pdf"),
                                                          (PDFIngestor, "DogQuotesTXT.txt"),
                                                          (TXTIngestor, "DogQuotesCSV.csv")])
    def test_negtive_ingestors_by_type(self, ingestor_type: str, file_name: str, quotes_root) -> None:
        file_path = quotes_root / file_name
        ...
        assert exc_info.value.message == f"Failed to ingest. This is not a {ingestor_file_type} file."

    @pytest.mark.parametrize("file_name", ["DogQuotesCSV.csv", "DogQuotesDOCX.docx",
                                           "DogQuotesPDF.pdf", "DogQuotesTXT.txt"])
    def test_ingestor(self, file_name: str, quotes_root) -> None:
        file_path = quotes_root / file_name
    ...
    def test_no_ingestor(self, img_root) -> None:
